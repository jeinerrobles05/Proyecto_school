...
Route::group([], function () {
    ...
    Route::group(['prefix' => '/tasks'], function () {
        ...
        Route::get('created', ['uses' => 'TasksController@created', 'middleware' => ['api.level-access:teacher']]);
        Route::get('not_participated', ['uses' => 'TasksController@notParticipated']);
        Route::get('{taskId}/result', ['uses' => 'TasksController@resultsByTask']);

        Route::get('results/my-results', ['uses' => 'TasksResultController@myResults']);
        Route::get('results/my-student-result', ['uses' => 'TasksResultController@myStudentResult', 'middleware' => ['api.level-access:teacher']]);
        Route::get('results/{taskResultId}/status', ['uses' => 'TasksResultController@status']);
        //    Route::get('results/{taskResultId}/download', 'CertificatesController@download');
        Route::get('results/{taskResultId}/show', 'CertificatesController@makeCertificate');

        Route::post('results/{taskResultId}/review',
            ['uses' => 'TasksResultController@updateResult', 'middleware' => ['api.level-access:teacher']]);


        Route::get('/{id}/start', ['uses' => 'TasksResultController@start']);
        Route::post('{id}/store-result', ['uses' => 'TasksResultController@tasksStoreResult']);
    ...
    Route::get('/tasks/{task}', ['uses' => 'TasksController@show'])->name('task.show');
