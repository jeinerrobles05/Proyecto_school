(function ($) {
    ...
    $('body').on('change', '.js-task-question-locale', function (e) {
        e.preventDefault();

        const $this = $(this);
        const $form = $(this).closest('.task-questions-form');
        const locale = $this.val();
        const item_id = $this.attr('data-id');

        $this.addClass('loadingbar gray');

        const path = '/panel/tasks-questions/' + item_id + '/getQuestionByLocale?locale=' + locale;
        ...
        $.get(path, function (result) {
            ...
            if (question.type === 'descriptive') {
                ...
                if (question.tasks_questions_answers && question.tasks_questions_answers.length) {
                    var answers = question.tasks_questions_answers;
