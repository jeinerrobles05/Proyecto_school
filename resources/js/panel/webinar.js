(function ($) {
    ...
    * */
    ...
    $('body').on('click', '.js-add-course-content-btn, .add-new-interactive-file-btn', function (e) {
        ...
        switch (type) {
            ...
            case 'task':
                ...
                newTaskForm.find('.chapter-input').val(chapterId);
                html = newTaskForm.html();
     ...
     * add webinar Task
     * */
    $('body').on('click', '#webinarAddTask', function (e) {
        e.preventDefault();
        const key = randomString();

        let add_task = $('#newTaskForm').html();
        add_task = add_task.replaceAll('record', key);

        $('#tasksAccordion').prepend(add_task);

        feather.replace();
    });

    $('body').on('click', '.js-save-task', function (e) {
        e.preventDefault();
        const $this = $(this);
