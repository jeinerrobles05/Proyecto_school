(function ($) {
    ...
    $('body').on('change', '.search-webinar-select2', function (e) {
        const $this = $(this);
        const webinarId = $this.val();

        const path = '/admin/enrollments/getAllByWebinarId/' + webinarId;

        var $groupSelection = $('.js-ajax-course_group_id');

        $groupSelection.addClass('loadingbar gray');

        $.get(path, function (result) {
            if (result && result.groups) {
                var html = '<option value="">' + "Seleccione un grupo" + '</option>';
