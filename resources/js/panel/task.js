(function ($) {
    ...
    $('body').on('click', '#add_multiple_question', function (e) {
        ...
        var multipleQuestionModal = $('.multipleQuestionModal' + task_id);
    ...
    $('body').on('click', '#add_descriptive_question', function (e) {
        ...
        var multipleQuestionModal = $('.descriptiveQuestionModal' + task_id);
        var clone = multipleQuestionModal.clone();
    ...
    $('body').on('click', '.save-question', function (e) {
        e.preventDefault();
        const $this = $(this);
        let form = $this.closest('.task-questions-form');
    ...
    $('body').on('click', '.edit_question', function (e) {
        ...
        $.get('/panel/tasks-questions/' + question_id + '/edit', function (result) {
    ...
    $('body').on('click', '.js-submit-task-form', function (e) {
        e.preventDefault();
        const $this = $(this);

        let form = $this.closest('.task-form');
        let data = serializeObjectByTag(form);
        let action = form.attr('data-action');
    ...
    $('body').on('change', '.js-ajax-webinar_id', function (e) {
        ...
        const path = '/panel/tasks/getAllByWebinarId/' + webinarId;

        var $groupSelection = $('.js-ajax-group_id');

        $groupSelection.addClass('loadingbar gray');

        $.get(path, function (result) {
            if (result && result.groups) {
                var html = '<option value="">' + tasksSectionLang + '</option>';
                ...
                $groupSelection.removeClass('loadingbar gray');

                $groupSelection.html(html);
            }
        });
    });
    ...
    $(document).ready(function () {
        ...
        function updateToDatabase(table, taskId, idString) {
            $.post('/panel/tasks/'+ taskId +'/order-items', {table: table, items: idString}, function (result) {
            ...
            if (target.length) {
                target.sortable({
                    ...
                    update: function (e, ui) {
                        ...
                        updateToDatabase(table, taskId, sortData.join(','))
                    }
                });
            }
        }
