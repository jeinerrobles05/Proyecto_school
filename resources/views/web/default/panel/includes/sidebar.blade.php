...
<div class="panel-sidebar pt-50 pb-25 px-25" id="panelSidebar">
    ...
    <ul id="panel-sidebar-scroll" class="sidebar-menu  {{--sidebar-menu pt-10 @if(!empty($authUser->userGroup)) has-user-group @endif @if(empty($getPanelSidebarSettings) or empty($getPanelSidebarSettings['background'])) without-bottom-image @endif--}}" @if((!empty($isRtl) and $isRtl)) data-simplebar-direction="rtl" @endif>
        ...
        <li class="sidenav-item {{ (request()->is('panel/tasks') or request()->is('panel/tasks/*')) ? 'sidenav-item-active' : '' }}">
            <a class="d-flex align-items-center" data-toggle="collapse" href="#tasksCollapse" role="button" aria-expanded="false" aria-controls="tasksCollapse">
                ...
                <span class="font-14 text-dark-blue font-weight-500">{{ trans('panel.tasks') }}</span>
            </a>

            <div class="collapse {{ (request()->is('panel/tasks') or request()->is('panel/tasks/*')) ? 'show' : '' }}" id="tasksCollapse">
                <ul class="sidenav-item-collapse">
                    @if(trim($authUser->role_name) == "organization" || trim($authUser->role_name) == "teacher")
                        <li class="mt-5 {{ (request()->is('panel/tasks/new')) ? 'active' : '' }}">
                            <a href="/panel/tasks/new">{{ trans('task.new_task') }}</a>
                        </li>
                        <li class="mt-5 {{ (request()->is('panel/tasks')) ? 'active' : '' }}">
                            <a href="/panel/tasks">{{ trans('public.list') }}</a>
                        </li>
                        <li class="mt-5 {{ (request()->is('panel/tasks/results')) ? 'active' : '' }}">
                            <a href="/panel/tasks/results">{{ trans('public.results') }}</a>
                        </li>
                    @endif

                    @if(trim($authUser->role_name) == "user")
                        <li class="mt-5 {{ (request()->is('panel/tasks/my-results')) ? 'active' : '' }}">
                            <a href="/panel/tasks/my-results">{{ trans('public.my_results') }}</a>
                        </li>

                        <li class="mt-5 {{ (request()->is('panel/tasks/opens')) ? 'active' : '' }}">
                            <a href="/panel/tasks/opens">{{ trans('public.not_participated') }}</a>
                        </li>
                    @endif
