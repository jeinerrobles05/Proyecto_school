...
class WebinarStatisticController extends Controller
{
    public function index(Request $request, $webinarId)
    {
        ...
        if (!empty($webinar)) {
            ...
            $data = [
                ...
                'pendingTasksCount' => $this->getPendingTasksCount($webinarId),
                ...
                'tasksAverageGrade' => $this->getTasksAverageGrade($webinarId),
                ...
                'taskStatusChart' => $this->handleTaskStatusChart($webinar),
    ...
    private function getStudents(Request $request, $webinar, $studentsIds, $gifts)
    {
        ...
        $tasksIds = $webinar->tasks->pluck('id')->toArray();
        ...
        foreach ($users as $user) {
            ...
            $user->passed_tasks = Task::whereIn('tasks.id', $tasksIds)
                ->join('tasks_results', 'tasks_results.task_id', 'tasks.id')
                ->select(DB::raw('count(tasks_results.id) as count'))
                ->where('tasks_results.user_id', $user->id)
                ->where('tasks_results.status', TasksResult::$passed)
                ->first()->count;
    ...
    private function getPendingTasksCount($webinarId)
    {
        return Task::where('webinar_id', $webinarId)
            ->where('status', 'active')
            ->whereHas('taskResults', function ($query) {
                $query->where('status', 'waiting');
            })
            ->count();
    }
    ...
    private function getTasksAverageGrade($webinarId)
    {
        $tasks = Task::where('webinar_id', $webinarId)
            ->join('tasks_results', 'tasks_results.task_id', 'tasks.id')
            ->select(DB::raw('avg(tasks_results.user_grade) as result_grade'))
            ->whereIn('tasks_results.status', ['passed', 'failed'])
            ->groupBy('tasks_results.task_id')
            ->get();

        return $tasks->avg('result_grade');
    }
    ...
    private function handleTaskStatusChart($webinar)
    {
        $labels = [
            trans('quiz.passed'),
            trans('public.pending'),
        ...
        foreach ($tasks as $task) {
            $passed = $task->taskResults()->where('status', TasksResult::$passed)->count();
            $pending = $task->taskResults()->where('status', TasksResult::$waiting)->count();
            $failed = $task->taskResults()->where('status', TasksResult::$failed)->count();
    ...
    public function getCourseProgressForStudent($webinar, $userId)
    {
        ...
        $tasksStat = $webinar->getTasksLearningProgressStat($userId);

        $passed = $filesStat['passed'] + $sessionsStat['passed'] + $textLessonsStat['passed'] + $assignmentsStat['passed'] + $quizzesStat['passed'] + $tasksStat['passed'];
        $count = $filesStat['count'] + $sessionsStat['count'] + $textLessonsStat['count'] + $assignmentsStat['count'] + $quizzesStat['count'] + $tasksStat['count'];
