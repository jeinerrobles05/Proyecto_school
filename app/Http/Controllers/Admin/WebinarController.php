...
*/
        ...
        if ($curso) {
            ...
            $nombreGrupo = 'Grupo ' . ($cantidadGrupos + 1);
            ...
            if (!empty($grupo)) {
                if ($request->has('horario') && $request->input('horario') !== null) {
                    ...
                    if (is_array($horario) && count($horario) > 0) {
                        $this->storeSchedule($horario, $grupo->id);
                    }
                }
    ...
    public function storeSchedule($horarios, $id)
    {
        foreach ($horarios as $horario) {
            $item = new Schedule();
            $item->day	=$horario['day'];
            $item->end_time	=$horario['end_time'];
            $item->start_time	=$horario['start_time'];
            $item->course_group_id =$id;
            $item->save();
        }
    }
