...
class WebinarContentController extends Controller
{
    ...
    public function tasks($webinar_id)
    {
        $tasks = Task::where('webinar_id', $webinar_id)->where('status', 'active')->get();

        return apiResponse2(1, 'retrieved', trans('api.public.retrieved'), TaskResource::collection($tasks));
    }
