...
class WebinarController extends Controller
{
    ...
    public function course($slug, $justReturnData = false)
    {
        ...
        if (!empty($course->tasks)) {
            $webinarContentCount += $course->tasks->count();
        }
        ...
        $tasks = $course->tasks->whereNull('chapter_id');

        if ($user) {
            $quizzes = $this->checkQuizzesResults($user, $quizzes);
            $tasks = $this->checkTasksResults($user, $tasks);
                ...
                foreach ($course->chapters as $chapter) {
                    if (!empty($chapter->chapterItems) and count($chapter->chapterItems)) {
                        foreach ($chapter->chapterItems as $chapterItem) {
                            ...
                            if (!empty($chapterItem->task)) {
                                $chapterItem->task = $this->checkTaskResults($user, $chapterItem->task);
                            }
                        }
                    }
                }
            ...
            if (!empty($course->tasks) and count($course->tasks)) {
                $course->tasks = $this->checkTasksResults($user, $course->tasks);
            }
        }
    ...
    private function checkTasksResults($user, $tasks)
    {
        $canDownloadCertificate = false;

        foreach ($tasks as $task) {
            $task = $this->checkTaskResults($user, $task);
        }
    ...
    private function checkTaskResults($user, $task)
    {
        $canDownloadCertificate = false;
        ...
        if (count($userTaskDone)) {
            $task->user_grade = $userTaskDone->first()->user_grade;
            $task->result_count = $userTaskDone->count();
            $task->result = $userTaskDone->first();
            ...
            $task->result_status = $status_pass ? TasksResult::$passed : $userTaskDone->first()->status;


        }

        if (!isset($task->attempt) or (count($userTaskDone) < $task->attempt and $task->result_status !== TasksResult::$passed)) {
            $canTryAgainTask = true;
        }

        $task->can_try = $canTryAgainTask;
