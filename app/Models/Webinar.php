...
class Webinar extends Model implements TranslatableContract
{
    ...
    {
        return $this->hasMany('App\course_group', 'curso_id', 'id');
    }
    ...
    {
        return $this->hasMany('App\Models\Task', 'webinar_id', 'id');
    }
    ...
    public function getTasksLearningProgressStat($userId = null)
    {
        $passed = 0;
        ...
        foreach ($tasks as $task) {
            $taskHistory = TasksResult::where('task_id', $task->id)
                ->where('user_id', $userId)
                ->where('status', TasksResult::$passed)
                ->first();
    ...
    public function getProgress($isLearningPage = false)
    {
        ...
        if (
            ...
            $tasksStat = $this->getTasksLearningProgressStat($user_id);
    ...
    public function sendNotificationToAllStudentsForNewTaskPublished($task,$template)
    {
        $studentsIds = $this->getStudentsIds();

        // Obtener el ID del grupo asociado a la tarea
        $taskGroupId = $task->group_id;

        // Filtrar los IDs de estudiantes para obtener solo aquellos asociados al grupo de la tarea
        $filteredStudentIds = GroupStudent::where('group_id', $taskGroupId)
        ...
        foreach ($filteredStudentIds as $studentId) {
            sendNotificationTask($template, $notifyOptions, $studentId);
        }
       ...
       // if (count($studentsIds)) {
        ...
        foreach ($gifts as $gift) {
            ...
            if (empty($user)) {
                sendNotificationToEmailTask($template, $notifyOptions, $gift->email);
            }
        }
    }
}
